	...
	public static function ajax_edit_composite_in_order() {
		...
		if ( $posted_configuration !== $current_configuration ) {
			...
			if ( is_wp_error( $added_to_order ) ) {
				...
				if ( $notice ) {
					$message = sprintf( _x( '%1$s %2$s', 'edit composite in order: formatted validation message', 'woocommerce-composite-products' ), $message, $notice );
				}
			...
			// Remove old items.
				...
				foreach ( $items_to_remove as $remove_item ) {
					...
					if ( WC_CP_Core_Compatibility::is_wc_version_gte( '3.6' ) ) {
						...
						if ( $changed_stock && isset( $changed_stock[ 'from' ] ) ) {
							$composited_item_id                   = $remove_item->get_meta( '_composited_item_id' );
							$pre_stock_map[ $composited_item_id ] = $changed_stock[ 'from' ];
						}
					}
				...
				if ( WC_CP_Core_Compatibility::is_wc_version_gte( '3.6' ) ) {
					...
					$composited_order_items = wc_cp_get_composited_order_items( $order->get_item( $added_to_order ), $order );
					foreach ( $composited_order_items as $order_item_id => $order_item ) {
						$product = $order_item->get_product();
						$qty     = $order_item->get_quantity();

						if ( $product->managing_stock() ) {
							$composited_item_id = $order_item->get_meta( '_composited_item_id' );
							$old_stock          = isset( $pre_stock_map[ $composited_item_id ] ) ? $pre_stock_map[ $composited_item_id ] : $product->get_stock_quantity();
							$new_stock          = wc_update_product_stock( $product, $qty, 'decrease' );

							if ( $old_stock && $old_stock !== $new_stock ) {
								$order->add_order_note( sprintf( __( 'Adjusted %s stock', 'woocommerce-composite-products' ), $product->get_formatted_name() ) . ' (' . $old_stock . '&rarr;' . $new_stock . ')', false, true );
							}
